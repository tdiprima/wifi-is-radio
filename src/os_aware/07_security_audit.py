...
# get_gateway renamed to get_default_gateway and imported from network_os_utils
    ...
    for port, name, desc in open_ports:
        if port == 23:
            ...
            findings.append(
                (
                    "MEDIUM",
                    f"{name} (port {port}) open on router",
                    desc,
        ...
        for mac, ips in mac_to_ips.items():
            if len(ips) > 1:
                findings.append(
                    (
                        "HIGH",
                        "Multiple IPs sharing same MAC address",
...
def check_open_services():
    ...
    if any(s[2] == "Telnet" for s in insecure_services):
        findings.append(
            (
                "HIGH",
                "Telnet services found on network",
