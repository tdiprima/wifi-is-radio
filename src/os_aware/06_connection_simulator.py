...
"""
    ...
    for ssid, security, signal, bssid in networks:
        print_packet(
            "in", "BEACON", f"SSID: {ssid}, Security: {security}, Signal: {signal}dBm"
        )
...
"""
    ...
    print_packet(
        "in", "AUTH RESPONSE", "Algorithm: Open System, Seq: 2, Status: Successful"
    )
...
"""
    ...
    print("-" * 50)
    print_packet(
        "out", "ASSOC REQUEST", "SSID: HomeNetwork_5G, Capabilities: 802.11n, 802.11ac"
    )
    print_packet(
        "in",
        "ASSOC RESPONSE",
...
"""
    ...
    print_packet("in", "DHCP ACK", "Confirmed. Lease time: 24 hours")
