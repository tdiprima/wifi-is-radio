...
def categorize_domain(domain: str) -> str:
    ...
    categories = {
        ...
        "Apple Services": ["apple.com", "icloud", "itunes", "mzstatic"],
        "Microsoft Services": [
            "microsoft",
            "windows",
            "office",
        ...
        "Google Services": ["google", "googleapis", "gstatic", "youtube", "gmail"],
        "IoT/Smart Home": [
            "nest",
            "ring.com",
            "smartthings",
            ...
            if result and result["queries"] and not result["is_response"]:
                for query in result["queries"]:
                    ...
                    if domain and domain not in domains_seen:
                        ...
                        display_domain = (
                            domain[:33] + ".." if len(domain) > 35 else domain
                        )

                        print(
                            f"{result['time']:<10} {result['src_ip']:<16} {display_domain:<35} {category}"
                        )

        # Capture DNS packets (port 53)
        sniff(iface=interface, filter="udp port 53", prn=packet_callback, store=False)
